...
Item {
    ...
    function toplevelForClient(client) {
        return ToplevelManager.toplevels.values.find(toplevel =>
            `0x${toplevel.HyprlandToplevel?.address}` === client.address
        ) ?? null
    }
    ...
    function iconForClient(client) {
        ...
        return Quickshell.iconPath(
            entry?.icon || appId.toLowerCase(),
            "application-x-executable"
        )
    }

    onOverviewOpenChanged: {
        if (overviewOpen) refresh()
    }
    ...
    Process {
        ...
        stdout: StdioCollector {
            onStreamFinished: {
                try {
                    root.clients = JSON.parse(text).filter(client =>
                        client.mapped !== false
                        && !client.hidden
                    )
                } catch (error) {
        ...
        Repeater {
            ...
            Rectangle {
                ...
                MouseArea {
                    ...
                    onClicked: {
                        Hyprland.dispatch(
                            `hl.dsp.focus({ workspace = ${workspace.workspaceId} })`
                        )
                        root.closeRequested()
                    }
                }
                ...
                Item {
                    ...
                    Repeater {
                        model: ScriptModel {
                            values: root.clients.filter(client =>
                                client.workspace.id === workspace.workspaceId
                            )
                        }
                        ...
                        delegate: Item {
                            ...
                            readonly property real homeX: Math.max(
                                0,
                                (client.at[0] - clientMonitor.x)
                                    / clientMonitor.width
                                    * parent.width
                            )
                            readonly property real homeY: Math.max(
                                0,
                                (client.at[1] - clientMonitor.y)
                                    / clientMonitor.height
                                    * parent.height
                            )
                            readonly property real previewWidth: Math.max(
                                42,
                                Math.min(
                                    client.size[0] / clientMonitor.width * parent.width,
                                    parent.width - homeX
                                )
                            )
                            readonly property real previewHeight: Math.max(
                                30,
                                Math.min(
                                    client.size[1] / clientMonitor.height * parent.height,
                                    parent.height - homeY
                                )
                            )
                            ...
                            DragHandler {
                                ...
                                onActiveChanged: {
                                    ...
                                    if (
                                        targetWorkspace !== -1
                                        && targetWorkspace !== preview.client.workspace.id
                                    ) {
                                        Hyprland.dispatch(
                                            `hl.dsp.window.move({ workspace = ${targetWorkspace}, follow = false, window = "address:${preview.client.address}" })`
                                        )
                                        refreshTimer.restart()
                                    }
                            ...
                            TapHandler {
                                ...
                                onTapped: {
                                    Hyprland.dispatch(
                                        `hl.dsp.focus({ window = "address:${preview.client.address}" })`
                                    )
                                    root.closeRequested()
                                }
                            }
                            ...
                            TapHandler {
                                ...
                                onTapped: {
                                    Hyprland.dispatch(
                                        `hl.dsp.window.close({ window = "address:${preview.client.address}" })`
                                    )
                                    refreshTimer.restart()
                                }
                            }
